...
async function loadPoem() {
  ...
  if (titleEl)
    titleEl.innerHTML = '<div class="skeleton h-8 w-48 mx-auto rounded"></div>';
  if (bodyEl)
    bodyEl.innerHTML = Array(3)
      .fill(
        `
    <div class="skeleton h-6 w-full rounded mb-3"></div>
    <div class="skeleton h-6 w-3/4 mx-auto rounded mb-6"></div>`,
      )
      .join("");
  ...
  try {
    ...
    if (categoryEl) {
      categoryEl.textContent =
        CATEGORY_LABELS[poem.category] || poem.category || "";
    }
...
function renderPoemBody(poem, container) {
  ...
  if (!verses.length) {
    container.innerHTML = `<p class="font-poem text-2xl leading-loose text-center whitespace-pre-wrap">${escapeHtml(bodyText)}</p>`;
    return;
  }
